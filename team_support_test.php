        ...
        if (!empty($row['employee_id'])) {
            ...
            while ($row2 = $mDB2->fetchRow(2)) {
                $member_teamwork[] = [
                    ...
                    'construction_site' => $row2['team_name'],
    ...
    foreach ($teamwork_map as $id => $dispatches) {
        ...
        foreach ($dispatches as $dispatch_day => $entry) {
            ...
            if (isset($entry['attendance_time']) && $entry['attendance_status'] === "支援") {
                $sheet->setCellValue("{$columnLetter}{$current_row}", $entry['attendance_time'] . "hr");
                $total_hours += $entry['attendance_time']; // 累加總時數
                $daily_totals[$dispatch_day - $start_day + 1] += $entry['attendance_time']; // 每日加總
            }
            ...
            if (isset($entry['transition']) && $entry['transition'] === "Y" && isset($entry['transition_team'])) {
                $transition_hours = calculateLeaveHours($entry['transition_start'], $entry['transition_end']);
                $sheet->setCellValue("{$columnLetter}" . ($current_row + 2), "轉場:" . $entry['transition_team'] . $transition_hours . "hr");
